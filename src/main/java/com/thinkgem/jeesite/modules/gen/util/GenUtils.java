...
public class GenUtils {
	...
	public static void initColumnField(GenTable genTable){
		...
		for (GenTableColumn column : genTable.getColumnList()){
			...
			column.setCsvPosition(position++);
...
//		}
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme){
		...
		// add ioArtifact by jeffen@pactera 2015/6/5 start
		model.put("ioArtifact", genScheme.getIoArtifact());
		model.put("projectPath", genScheme.getProjectPath());
		// add ioArtifact by jeffen@pactera 2015/6/5 end
		// add row-number by jeffen@pactera 2015/9/16 start
		model.put("showRowNo", genScheme.getShowRowNo()?"1":null);
		// add row-number by jeffen@pactera 2015/9/16 end
		// add row-checkbox by jeffen@pactera 2015/9/25 start
		model.put("showRowCheckBox", genScheme.getShowRowNo()?"1":null);
		// add row-checkbox by jeffen@pactera 2015/9/25 end
		// add row-row-copy by jeffen@pactera 2015/10/4 start
		model.put("showRowCopyFunction", genScheme.getShowRowCopyFunction()?"1":null);
		model.put("showPhysicalDeleteFunction", genScheme.getShowPhysicalDeleteFunction()?"1":null);
		// add row-row-copy by jeffen@pactera 2015/10/4 end
	...
	public static String generateToFile(GenTemplate tpl, Map<String, Object> model, boolean isReplaceFile){
		// 获取工程路径
		String projectPath = (String) model.get("projectPath");
		if (StringUtils.isEmpty(projectPath)){
			projectPath = Global.getProjectPath();
		}
		// 获取生成文件
