...
public class GenTableColumn extends DataEntity<GenTableColumn> {
	...
	// add table column position by jeffen@pactera 2015/6/4 start
	private String isCsv; // 是否CSV字段（1：CSV字段）
	private String csvColumnAccessType; // CSV列数据访问方式：[READ]导入只读，但不导该出字段；[READ_WRITE]导入只读，并导该出字段；[WRITE]导入文件不含该字段，但导该出字段
	private Integer csvPosition; // CSV文件列位置
	private String orderType; // 结果排序方式（非排序字段、升序ASC、降序DESC）
	private String defaultValue; // 默认值(CSV文件模版数据用例)
	private String format; // 格式化(#,##0.0;yyyy/MM/dd;HH:mm:ss)
	...
	public String getCsvColumnAccessType() {
		return csvColumnAccessType;
	}

	public void setCsvColumnAccessType(String csvColumnAccessType) {
		this.csvColumnAccessType = csvColumnAccessType;
	}
	...
	public void setCsvPosition(Integer csvPosition) {
	...
	// add table column position by jeffen@pactera 2015/6/4 end

	// add excel io by jeffen@pactera 2015/9/11 start
	private String isExcel; // 是否EXCEL字段
	private String excelValue; // EXCEL导出字段名（默认调用当前字段的“get”方法，如指定导出字段为对象，请填写“对象名.对象属性”，例：“area.name”、“office.name”）
	private String excelTitle; // EXCEL导出字段标题（需要添加批注请用“**”分隔，标题**批注，仅对导出模板有效）
	private String excelType; // EXCEL字段类型（0：导出导入；1：仅导出；2：仅导入）
	private String excelAlign; // EXCEL导出字段对齐方式（0：自动；1：靠左；2：居中；3：靠右）
