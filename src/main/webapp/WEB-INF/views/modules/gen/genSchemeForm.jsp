...
<body>
	...
	<form:form id="inputForm" modelAttribute="genScheme" action="${ctx}/gen/genScheme/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="category" class="required input-xlarge" onchange="location.href='${ctx}/gen/genScheme/form?id=${genScheme.id}&category='+this.value">
		...
		<div class="control-group">
			<label class="control-label">模版清单查看:</label>
			<div class="controls">
				<form:select path="templateFiles" class="input-xlarge" multiple="true">
					<form:options items="${config.templateFiles}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
				<span class="help-inline">
					清单目录：src/main/resources/templates/modules/gen/*
				</span>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">工程路径:</label>
			<div class="controls">
				<form:input path="projectPath" htmlEscape="false" maxlength="500" class="required input-xlarge" />
				<span class="help-inline">默认配置jeesite.properties：projectPath=${fns:getConfig('projectPath')}</span>
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span class="help-inline">建议模块包：com.pactera.*.modules</span>
			</div>
		</div>
		<div class="control-group">
		...
		<%// add io_artifact by jeffen@pactera 2015/6/5 start %>
		<div class="control-group">
			<label class="control-label">导入导出工具包:</label>
			<div class="controls">
				<form:select path="ioArtifact" class="input-xlarge">
					<form:option value="" label="${fns:getLang('common.please.select', '---请选择---')}"/>
					<form:options items="${fns:getDictList('io_artifact')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
		</div>
		<%// add io_artifact by jeffen@pactera 2015/6/5 end %>
		
		<div class="control-group">
			<label class="control-label">功能选项:</label>
			<div class="controls">
				<u>表格：</u>
				<br />
				<form:checkbox path="showRowNo" label="是否显示行号" />
				<br />
				<form:checkbox path="showRowCheckBox" label="[edit]是否显示行复选(带批量删除)" title="[shiro:hasPermission]module:edit" />
				<br />
				<form:checkbox path="showRowCopyFunction" label="[dba]是否显示复制功能" title="[shiro:hasPermission]module:dba" />
				<br />
				<form:checkbox path="showPhysicalDeleteFunction" label="[dba]是否显示物理删除功能" title="[shiro:hasPermission]module:dba" />
				<br />
			</div>
		</div>
		<div class="control-group">
